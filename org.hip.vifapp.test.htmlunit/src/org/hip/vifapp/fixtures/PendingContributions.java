...
/**
 * Fitnesse test to query the status quo of pending contributions before the request for publication is made.
 *
 * @author Luthiger
 * Created 20.02.2009 
 */
public class PendingContributions extends VifForumFixture {
	private final static String CHANGE_TO = "org.hip.vif.forum.groups.showUnpublished&groupID=%s";
	 ...
	 * @param inPort
	 */
	public PendingContributions(String inPort, String inUserName, String inGroupName) {
		super(inPort);
		userName = inUserName;
	...
	private Object evalRow(HtmlTableRow inRow) {
		return list(
				list("contribution", inRow.getCell(2).asText()),
				list("state", inRow.getCell(3).asText())
				);
	}
