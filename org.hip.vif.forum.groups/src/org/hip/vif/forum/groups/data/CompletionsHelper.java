 ...
 * Reason: the query to retrieve the question's completions may retrieve each completion twice, first with the author
 * data and second with the reviewer data.<br/>
...
public class CompletionsHelper {
     ...
     * @throws VException
     * @throws SQLException */
    public static List<Completion> getNormalizedCompletions(final QueryResult inCompletions) throws VException,
            SQLException {
        ...
        while (inCompletions.hasMoreElements()) {
            final GeneralDomainObject lModel = inCompletions.nextAsDomainObject();
            final Long lCompletionID = BeanWrapperHelper.getLong(CompletionHome.KEY_ID, lModel);
    ...
    public static class Completion {
        ...
        private void addContributor(final GeneralDomainObject inModel) {
            final MemberWrapper lContributor = AuthorReviewerRenderHelper.wrapMember(inModel);
            final Integer lType = BeanWrapperHelper.getInteger(ResponsibleHome.KEY_TYPE, inModel);
