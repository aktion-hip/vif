 ...
 *
 * @author Luthiger Created: 12.06.2011 */
...
public class BibliographyView extends AbstractContributionView {
     ...
     * @throws SQLException
     * @throws VException */
    public BibliographyView(final GeneralDomainObject inText, final QueryResult inDownloads, final Member inAuthor,
            final Member inReviewer,
            final String inKeyType, final boolean inIsGuest, final IPluggableWithLookup inTask) throws VException,
            SQLException {
        ...
        // add author/reviewer
        final AuthorReviewerRenderer lRenderer = AuthorReviewerRenderHelper.createRenderer(inAuthor, inReviewer,
                inTask, !inIsGuest);
    ...
    public BibliographyView(final Text inText, final QueryResult inDownloads, final QueryResult inAuthors,
        ...
        final FormCreator lForm = BibliographyViewHelper.createBiblioForm(inText, new UploadComponent(inDownloads,
                inTask), getState(BeanWrapperHelper.getString(TextHome.KEY_STATE, inText), inCodeList, lMessages));
            ...
            public void buttonClick(final ClickEvent inEvent) {
                try {
                    ...
                    if (!inTask.saveText(inText, inCreateVersion)) {
                        Notification.show(
                                lMessages.getMessage("errmsg.save.general"), Type.WARNING_MESSAGE); //$NON-NLS-1$
                    }
                }
