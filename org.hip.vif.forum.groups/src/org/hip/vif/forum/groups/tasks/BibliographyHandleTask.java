...
public class BibliographyHandleTask extends AbstractBibliographyTask implements ValueChangeListener {
    ...
    protected Component runChecked() throws RiplaException {
        ...
        try {
            ...
            final AutoCompleteHelper lHelper = new AutoCompleteHelper(BOMHelper.getTextHome(), lKey);
    ...
    public boolean searchFor(final Object inTitle, final Object inAuthor) {
        try {
            final QueryResult lSearchResult = BOMHelper.getTextHome().selectTitleOrAuthor(
                    inTitle == null ? "" : inTitle.toString(), //$NON-NLS-1$
                            inAuthor == null ? "" : inAuthor.toString()); //$NON-NLS-1$
            if (lSearchResult.hasMoreElements()) {
                // display list of search results
                texts = ContributionContainer.createData(lSearchResult,
                        CodeListHome.instance().getCodeList(QuestionState.class, getAppLocale().getLanguage()));
                return true;
            }
            else {
    ...
    public void addBibliography() {
        ...
        final IMessages lMessages = Activator.getMessages();
        if (lCount == 0) {
            showNotification(lMessages.getMessage("errmsg.save.general"), Type.ERROR_MESSAGE); //$NON-NLS-1$
            return;
        }
        showNotification(
                lMessages.getMessage(lCount == 1 ? "msg.bibliography.addedS" : "msg.bibliography.addedM"), Type.TRAY_NOTIFICATION); //$NON-NLS-1$ //$NON-NLS-2$
        sendEvent(QuestionShowTask.class);
    }

    private Long splitTextID(final String inTextID) {
    ...
    public void valueChange(final ValueChangeEvent inEvent) {
        ...
        if (lProperty instanceof Table) {
            ...
            if (lValue instanceof ContributionWrapper) {
                final Long lID = splitTextID(((ContributionWrapper) lValue).getID());
