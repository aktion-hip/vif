 ...
 * Created: 10.07.2010
 */
public abstract class AbstractCompletionTask  extends AbstractVIFTask {
	...
	public Component runChecked() throws VException {
		...
		try {
			...
			Long lQuestionID = getCompletionsQuestionID();
			DomainObject lQuestion = BOMHelper.getQuestionHome().getQuestion(lQuestionID.toString());
			QueryResult lCompletions = BOMHelper.getJoinCompletionToMemberHome().getAuthorView(lQuestionID, lActorID);			
			...
			return new CompletionView(getCompletionText(), getActCompletionID(), lQuestion, lCompletions, lGroup, lCodeList, this); //$NON-NLS-1$
		} 
		catch (Exception exc) {
