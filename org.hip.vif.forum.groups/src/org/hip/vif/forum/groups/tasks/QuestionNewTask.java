...
public class QuestionNewTask extends AbstractQuestionTask {
	...
	public boolean saveQuestion(String inQuestion, String inRemark) {
		try {
			Question lQuestionBOM = (Question)BOMHelper.getQuestionHome().create();
			lQuestionBOM.ucNew(cleanUp(inQuestion), cleanUp(inRemark), getQuestionID(), getGroupID().toString(), getActor().getActorID());
			showNotification(Activator.getMessages().getMessage("msg.question.create"), Notification.TYPE_TRAY_NOTIFICATION); //$NON-NLS-1$
