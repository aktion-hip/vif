 ...
 *
 * @author Luthiger Created: 30.08.2011 */
public class SwitchHelper {
    ...
    public SwitchHelper(final boolean inHasSubscription, final boolean inHasBookmark, final Long inQuestionID,
    ...
    public String getSubscriptionCaption() {
        return hasSubscription ? messages.getMessage(KEY_SUBSCRIPTION_DELETE) : messages
                .getMessage(KEY_SUBSCRIPTION_CREATE);
    }
    ...
    public boolean createBookmark(final String inBookmarkText) {
        if (!isInitialized)
            return false;
        if (hasBookmark)
            return false;
     ...
     * 
     * @param inLocal boolean <code>true</code> if the subscription is only local for the selected question,
     *            <code>false</code> if the user subscibed for the whole subtree
     * @return boolean <code>true</code> if the subscription could be saved successfully */
    public boolean createSubscription(final boolean inLocal) {
        if (!isInitialized)
            return false;
        if (hasSubscription)
            return false;
