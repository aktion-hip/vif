...
public class AuthorReviewerRenderHelper {
	 ...
	 *            {@link IPluggableTask}
	 * @param makeClickable
	 *            boolean if <code>true</code>, the names are rendered clickable
	...
	public static AuthorReviewerRenderer createRenderer(
			...
			final IPluggableTask inTask, final boolean makeClickable)
	...
	public static AuthorReviewerRenderer createRenderer(final Member inAuthor,
			final Member inReviewer, final IPluggableTask inTask,
	...
	public static class AuthorReviewerRenderer {
		...
		public Component render(final String inAuthorLbl,
			...
			return new Label(String.format(TMPL_TITLE, STYLE_CLASS,
					lAuthorRevierDate), Label.CONTENT_XHTML); //$NON-NLS-1$
		}
	...
	private static class ClickabelAuthorReviewerRenderer extends
		...
		private final Label SEP_COMMA = RiplaViewHelper
				.makeUndefinedWidth(new Label(
						String.format(TMPL_SPACED, ","), Label.CONTENT_XHTML)); //$NON-NLS-1$
		private final IPluggableTask task;

		public ClickabelAuthorReviewerRenderer(final QueryResult inAuthors,
				final QueryResult inReviewers, final IPluggableTask inTask)
				throws VException, SQLException {
		...
		ClickabelAuthorReviewerRenderer(final GeneralDomainObject inAuthor,
				final IPluggableTask inTask) throws VException, SQLException {
			super(inAuthor);
			task = inTask;
		}

		ClickabelAuthorReviewerRenderer(final GeneralDomainObject inAuthor,
				final GeneralDomainObject inMember, final IPluggableTask inTask)
				throws VException, SQLException {
		...
		public Component render(final String inAuthorLbl,
			...
			out.addComponent(RiplaViewHelper.makeUndefinedWidth(new Label(
					String.format(
							"<span class=\"vif-contribution-date\">[%s]<span>", inDate), Label.CONTENT_XHTML))); //$NON-NLS-1$
			return out;
		}
