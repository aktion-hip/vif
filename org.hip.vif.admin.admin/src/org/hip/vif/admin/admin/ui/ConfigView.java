...
import org.hip.vif.web.util.GenericSelect;
import org.hip.vif.web.util.GenericSelect.IProcessor;
import org.hip.vif.web.util.Popup;
...
public class ConfigView extends AbstractAdminView {
	 ...
	 *            {@link ConfigurationItem} the wrapper item for the
	 *            configuration
	 * @param inTask
	 *            {@link ConfigTask}
	 * @throws IOException
	...
	public ConfigView(final ConfigurationItem inConfiguration,
		...
		lTable.addRow(createSubSubtitle(
				"admin.config.sub.database.ext", lMessages)); //$NON-NLS-1$
		lTable.addRow(
		...
		lTable.addRow(createSubSubtitle(
				"admin.config.sub.database.ldap", lMessages)); //$NON-NLS-1$
		lTable.addRow(
			...
			lTable.addRow(
					...
					createInput(
							GenericSelect.getSelection(
									...
									WIDTH, false, null),
							"admin.config.desc.logging.level", lMessages, lForm)); //$NON-NLS-1$ //$NON-NLS-2$
			lTable.addRow(
	...
	protected Layout getSmoke(final IMessages inMessages) {
		...
		lValues.addRow(new Label(String.format(VIFViewHelper.TMPL_TITLE,
				"vif-title", "Application VIF"), ContentMode.HTML)); //$NON-NLS-1$ //$NON-NLS-2$
		lValues.addRow("Release:", getVersion()); //$NON-NLS-1$ //$NON-NLS-2$
		lValues.addRow(new Label(String.format(VIFViewHelper.TMPL_TITLE,
				"vif-title", "Java"), ContentMode.HTML)); //$NON-NLS-1$ //$NON-NLS-2$
		...
		lValues.addRow(
				"VM&nbsp;Version:", System.getProperty("java.vm.version")); //$NON-NLS-1$ //$NON-NLS-2$
		lValues.addRow(
				"Runtime&nbsp;Version:", System.getProperty("java.runtime.version")); //$NON-NLS-1$ //$NON-NLS-2$
		...
		lValues.addRow(
				"Endorsed&nbsp;Directory:", System.getProperty("java.endorsed.dirs")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private Label createSubtitle(final String inMsgKey,
		...
		return new Label(String.format(VIFViewHelper.TMPL_TITLE,
				"vif-title", inMessages.getMessage(inMsgKey)), ContentMode.HTML); //$NON-NLS-1$
	}
	...
	private Label createSubSubtitle(final String inMsgKey,
		...
		return new Label(
				String.format(VIFViewHelper.TMPL_TITLE,
						"vif-emphasized", inMessages.getMessage(inMsgKey)), ContentMode.HTML); //$NON-NLS-1$
	}
	...
	private HorizontalLayout createInput(final AbstractField inInput,
		...
		final Label lDescription = new Label(
				String.format(
						...
						"vif-contribution-date", inMessages.getMessage(inMsgKey)), ContentMode.HTML); //$NON-NLS-1$
	...
	private AbstractField decorateField(final AbstractField inInput,
		...
		inInput.setRequiredError(inMessages.getFormattedMessage(
				"errmsg.field.not.empty", inFieldName)); //$NON-NLS-1$
