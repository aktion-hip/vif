...
/** Implements the Circular Buffer producer/consumer model for bytes. More information about this class is available
 * from <a target="_top" href= "http://ostermiller.org/utils/CircularByteBuffer.html">ostermiller.org</a>.
 * <p>
...
public class CircularByteBuffer {
    ...
    /** Make this buffer ready for reuse. The contents of the buffer will be cleared and the streams associated with
     * this buffer will be reopened if they had been closed.
     *
     * @since ostermillerutils 1.00.00 */
    public void clear() {
        synchronized (this) {
    ...
    /** Create a new buffer with given capacity. Writing to a full buffer will block until space is available rather
     * than throw an exception.
     * <p>
    ...
    protected class CircularByteBufferInputStream extends InputStream {
         ...
         * the next caller of a method for this input stream. The next caller might be the same thread or or another
         * thread.
         *
        ...
        public int available() throws IOException {
            synchronized (CircularByteBuffer.this) {
                if (inputStreamClosed) {
                    throw new IOException("InputStream has been closed, it is not ready."); //$NON-NLS-1$
                }
        ...
        /** Mark the present position in the stream. Subsequent calls to reset() will attempt to reposition the stream
         * to this point.
         * <p>
        ...
        public int read() throws IOException {
            while (true) {
                synchronized (CircularByteBuffer.this) {
                    if (inputStreamClosed) {
                        throw new IOException("InputStream has been closed; cannot read from a closed InputStream."); //$NON-NLS-1$
                    }
                ...
                try {
                    ...
                    throw new IOException("Blocking read operation interrupted."); //$NON-NLS-1$
                }
            }
        }
        ...
        public void reset() throws IOException {
            synchronized (CircularByteBuffer.this) {
                if (inputStreamClosed) {
                    throw new IOException("InputStream has been closed; cannot reset a closed InputStream."); //$NON-NLS-1$
                }
                ...
                synchronized (CircularByteBuffer.this) {
                    if (inputStreamClosed) {
                        throw new IOException(
                                "InputStream has been closed; cannot skip bytes on a closed InputStream."); //$NON-NLS-1$
                    }
                    final int available = CircularByteBuffer.this.available();
                    if (available > 0) {
    ...
    protected class CircularByteBufferOutputStream extends OutputStream {
        ...
        public void flush() throws IOException {
            synchronized (CircularByteBuffer.this) {
                if (outputStreamClosed) {
                    throw new IOException("OutputStream has been closed; cannot flush a closed OutputStream."); //$NON-NLS-1$
                }
                if (inputStreamClosed) {
                    throw new IOException("Buffer closed by inputStream; cannot flush."); //$NON-NLS-1$
                }
            }
         ...
         * @since ostermillerutils 1.00.00 */
        @Override
        public void write(final byte[] cbuf, int off, int len) throws IOException {
            while (len > 0) {
                synchronized (CircularByteBuffer.this) {
                    if (outputStreamClosed) {
                        throw new IOException("OutputStream has been closed; cannot write to a closed OutputStream."); //$NON-NLS-1$
                    }
                    if (inputStreamClosed) {
                        throw new IOException("Buffer closed by InputStream; cannot write to a closed buffer."); //$NON-NLS-1$
                    }
                    ...
                    final int firstLen = Math.min(realLen, buffer.length - writePosition);
                    final int secondLen = Math.min(realLen - firstLen, buffer.length - markPosition - 1);
                ...
                if (len > 0) {
                    try {
                        ...
                        throw new IOException("Waiting for available space in buffer interrupted."); //$NON-NLS-1$
                    }
                }
            }
        }
