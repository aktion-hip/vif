...
public class UseCaseComponent implements IUseCase, IPermissionRecords {
	 ...
	 * The menu defines the makers
	 * <code>{"menuStart", "additions", "menuEnd"}</code>.
	 * </p>
	 * 
	...
	public IPermissionRecord[] getPermissionRecords() {
		...
		outRecords[0] = new PermissionRecord(
				Constants.PERMISSION_REFRESH_INDEX,
				"Administration: Refresh the index for fulltext search.", //$NON-NLS-1$
				new int[] { RolesConstants.SU, RolesConstants.ADMINISTRATOR });
		outRecords[1] = new PermissionRecord(Constants.PERMISSION_SELECT_SKIN,
				"Administration: Select the skin.", //$NON-NLS-1$
				new int[] { RolesConstants.SU, RolesConstants.ADMINISTRATOR });
		...
		outRecords[4] = new PermissionRecord(
				Constants.PERMISSION_CONFIGURATION,
				"Administration: Application configuration.", //$NON-NLS-1$
				new int[] { RolesConstants.SU, RolesConstants.ADMINISTRATOR });
		outRecords[5] = new PermissionRecord(Constants.PERMISSION_UPGRADE,
				"Administration: Application upgrade.", //$NON-NLS-1$
				new int[] { RolesConstants.SU, RolesConstants.ADMINISTRATOR });
		return outRecords;
	}
