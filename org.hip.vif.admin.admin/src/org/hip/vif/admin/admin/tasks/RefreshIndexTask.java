...
/** Task to display the page with the administration tasks to reindex the database.
 *
 * @author Benno Luthiger */
@UseCaseController
public class RefreshIndexTask extends AbstractWebController {
    private static final String NL = "<br />"; //$NON-NLS-1$ // NOPMD
     ...
     *
     * @param inCheckBoxes {@link Collection}
            ...
            if (value != null && value) {
                if (Constants.INDEX_CONTENT.equals(lCheckBox.getData())) {
                    ...
                    out.append(lMessages
                            .getFormattedMessage("admin.reindex.feedback.content", (Object[]) lIndexer.refreshIndex())); //$NON-NLS-1$
                    out.append(NL);
                } else {
                    final VIFMemberIndexer lIndexer = new VIFMemberIndexer();
                    out.append(lMessages
                            .getFormattedMessage("admin.reindex.feedback.person", (Object[]) lIndexer.refreshIndex())); //$NON-NLS-1$
                    out.append(NL);
                }
            }
        }
