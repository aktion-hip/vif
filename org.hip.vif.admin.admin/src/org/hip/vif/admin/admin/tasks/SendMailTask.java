...
/**
 * Shows the form to enter mail subject and text for a mail to the members or
 * participants of selected groups.
 * 
...
public class SendMailTask extends AbstractWebController {
	 ...
	 * 
	 * @param inSelected
	 *            Collection<GroupWrapper> the groups to whose members the mails
	 *            should be sent
	 * @param inSubject
	 *            String plain text
	 * @param inBody
	 *            String html (rich text)
	 * @return boolean <code>true</code> if the mails could be processed
	 *         successfully
	 */
		...
		try {
			...
			showNotification(
					Activator.getMessages().getMessage(
							"admin.send.mail.msg.success"), Type.TRAY_NOTIFICATION); //$NON-NLS-1$
