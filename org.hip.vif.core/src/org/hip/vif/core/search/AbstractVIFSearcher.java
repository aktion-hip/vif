...
public abstract class AbstractVIFSearcher extends AbstractSearching {
    ...
    /** Starts the search with the query term(s) provided and returns the hits as QueryResult.
     *
    ...
    public QueryResult search(final String inQueryTerm) throws IOException, ParseException, NoHitsException {
        final IndexReader lReader = getIndexReader();
        final IndexSearcher lSearcher = new IndexSearcher(lReader);
        final TopDocs lHits = lSearcher.search(parseQuery(inQueryTerm), NUMBER_OF_HITS);
     ...
     * @throws IOException
     * @throws ParseException */
    public QueryResult search(final String inName, final String inFirstName, final String inStreet,
            final String inPostal, final String inCity, final String inMail) throws NoHitsException, ParseException,
            IOException {
        final IndexReader lReader = getIndexReader();
        final IndexSearcher lSearcher = new IndexSearcher(lReader);
        ...
        final TopDocs lHits = lSearcher.search(lMain, NUMBER_OF_HITS);
        return getHitsQueryResult(processSearchResults(lHits, lSearcher,
                new String(lFeedback).substring(0, lFeedback.length() - 3)));
    }

    private BooleanQuery addQueryChecked(final String inValue, final String inFieldName, final BooleanQuery inQuery,
            final StringBuilder inFeedback) throws ParseException {
        if (inValue.length() == 0)
            return inQuery;

        inFeedback.append(inValue).append(" / ");
        final QueryParser lParser = new QueryParser(inFieldName, getAnalyzer());
        final Query lQuery = lParser.parse(inValue);
    ...
    /** Hook for subclasses: parses the query string. Subclasses decide which analyzer to use and which field to query.
     *
     * @param inQueryTerm String
     * @return Query
     * @throws ParseException */
     ...
     * @throws IOException
     * @throws NoHitsException */
    protected Document[] processSearchResults(final TopDocs inHits, final IndexSearcher inSearcher, final String inQuery)
            throws CorruptIndexException, IOException, NoHitsException {
        if (inHits.totalHits == 0)
            throw new NoHitsException(inQuery);
