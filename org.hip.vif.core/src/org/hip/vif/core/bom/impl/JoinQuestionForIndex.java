...
public class JoinQuestionForIndex extends DomainObjectImpl implements Indexable {
	...
	public void indexContent(IndexWriter inWriter) throws IOException, VException {
		...
		lDocument.add(AbstractSearching.IndexField.QUESTION_TEXT.createField(lFullText.add(get(QuestionHome.KEY_QUESTION).toString())));
		...
		lDocument.add(AbstractSearching.IndexField.GROUP_NAME.createField(get(JoinQuestionForIndexHome.JOIN_GROUP_NAME).toString()));
