...
public class RatingsHome extends DomainObjectHomeImpl {
     ...
     * @throws VException
     * @throws SQLException */
    public Long saveRating(final Long inRatingEventsID, final Long inRaterID, final int inCorrectness,
            final int inEfficiency, final int inEtiquette, final String inRemark) throws VException, SQLException {
        final KeyObject lKey = new KeyObjectImpl();
        lKey.setValue(KEY_RATINGEVENTS_ID, inRatingEventsID);
        lKey.setValue(KEY_RATER_ID, inRaterID);
        final Ratings lRatings = (Ratings) findByKey(lKey);
        final Long outRatedID = (Long) lRatings.get(KEY_RATED_ID);
     ...
     * @throws VException
     * @throws SQLException */
    public Long saveRating(final JoinRatingsToRater inRating, final Long inRaterID) throws VException, SQLException {
        return saveRating(getLong(KEY_RATINGEVENTS_ID, inRating), inRaterID, getInt(KEY_CORRECTNESS, inRating),
                getInt(KEY_EFFICIENCY, inRating), getInt(KEY_ETIQUETTE, inRating), getString(KEY_REMARK, inRating));
    }

    private Long getLong(final String inKey, final GeneralDomainObject inDomainObject) throws VException {
        final Object out = inDomainObject.get(inKey);
        return out == null ? new Long(0) : new Long(out.toString());
    }

    private Integer getInt(final String inKey, final GeneralDomainObject inDomainObject) throws VException {
        final Object out = inDomainObject.get(inKey);
        return out == null ? new Integer(0) : new Integer(out.toString());
    }

    private String getString(final String inKey, final GeneralDomainObject inDomainObject) throws VException {
        final Object out = inDomainObject.get(inKey);
        return out == null ? "" : out.toString();
    }

    /** Checks whether the rating with the specified ID is completed (by both parties), i.e. counts the number of open
     * ratings with the specified rating ID for the specified rated member.
     *
     * @param inRatingEventsID Long the rating's ID
     ...
     * @throws VException
     * @throws SQLException */
    public boolean checkRatingCompleted(final Long inRatingEventsID, final Long inRatedID) throws VException,
            SQLException {
        final KeyObject lKey = prepareKey(inRatingEventsID);
     ...
     * @throws VException
     * @throws SQLException */
    public boolean checkRatingCompleted(final Long inRatingEventsID) throws VException, SQLException {
        final KeyObject lKey = prepareKey(inRatingEventsID);
        return getCount(lKey) == 0;
    }

    private KeyObject prepareKey(final Long inRatingEventsID) throws VException {
     ...
     * @throws VException
     * @throws SQLException */
    public boolean isInvolved(final Long inRatingEventsID, final Long inMemberID) throws VException, SQLException {
        final KeyObject lKey = new KeyObjectImpl();
        lKey.setValue(KEY_RATINGEVENTS_ID, inRatingEventsID);
