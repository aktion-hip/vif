...
/**
 * <p>Abstract class providing basic functionality to send mails.<br/>
 * Implementations of the class can be used to send mails to a single person with at most one single <code>cc</code>.</p>
 * 
...
//	protected Locale locale;
	 ...
	 * @throws IOException
	 */
	public AbstractMail(VIFMember inReceiver) throws VException, IOException {
		super();
	 ...
	 * @throws IOException
	 */
	public AbstractMail(VIFMember inReceiver, VIFMember inSender) throws VException, IOException {
		super();
		init(inReceiver, inSender);
	}
	
	private void init(VIFMember inReceiver, VIFMember inSender) throws VException, IOException {
		receiver = inReceiver;
	...
	public void send() throws MailGenerationException {
		try {
			StringBuilder lSubject = new StringBuilder(subjectID).append(" ").append(getSubjectText());
			StringBuilder lAddress = createMailAddress();
			
			InternetAddress lAddressFrom = new InternetAddress(mailFrom);
			...
			lBody.append(lAddress).append("\n\n").append(getBody()).append(getMailGreetings());
			lMessage.addPart(new String(lBody));
			//html
			lBody = new StringBuilder();
			lBody.append("<p>").append(lAddress).append("</p>").append(getBodyHtml()).append(getMailGreetingsHtml());
	 ...
	 */
	protected String getMessage(IMessages inMessages, String inKey) {
	 ...
	 * @param inArg Object
	 */
	protected String getFormattedMessage(IMessages inMessages, String inKey, Object... inArgs) {
	...
	/**
	 * Creates the address of the mail, e.g. Dear XY,
	...
	protected StringBuilder createMailAddress() throws MailGenerationException {
		try {
			StringBuilder outAddress = new StringBuilder();
	 ...
	 * If more then one, addresses must be comma separated.
	 * 
	 * @param inAdress String Addition mail address(es)
	 */
	protected void addMailToAddress(String inAddress) {
		if (inAddress.length() == 0) return;
		
		mailTo += ", " +inAddress; 
	}
	
	protected String getUrlHtml(String inUrl, String inLabel) {
