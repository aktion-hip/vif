...
/**
 * <p>Abstract class providing generic functionality for mails sent
 * to notify a set of users.<br/>
 * Implementations of this class can be used to send mails to a set of persons.</p>
 * 
...
public abstract class AbstractNotification extends VObject implements IVIFMail {
	 ...
	 * @param isBcc boolean <code>True</code> if receiver addresses are set in the mailBcc, else they are set in the mailTo
	 */
	public AbstractNotification(InternetAddress[] inReceiverMails, boolean isBcc) {
		super();
	 ...
	 * @param isBCC boolean <code>True</code> if receiver addresses are set in the mailBcc, else they are set in the mailTo
	 */
	public AbstractNotification(Collection<AddressAdapter> inReceiverMails, boolean isBCC) {
		super();

		int i = 0;
		InternetAddress[] lAddresses = new InternetAddress[inReceiverMails.size()];
		for (AddressAdapter lAddress : inReceiverMails) {
			lAddresses[i++] = lAddress.getInternetAddress();
		}
		init(lAddresses, isBCC);
	}

	private void init(InternetAddress[] inReceiverMails, boolean isBcc) {
	...
	protected static String getPreference(String inKey) {
		try {
	...
	public void send() throws MailGenerationException {
		try {
			...
			InternetAddress lDummyAddress = new InternetAddress(mailFrom);
	 ...
	 * @param inArg Object
	 */
	protected String getFormattedMessage(IMessages inMessages, String inKey, Object inArg) {
		return inMessages.getFormattedMessage(inKey, inArg);
	}
	
	protected static StringBuffer getSubjectID() {
		String out = getPreference(PreferencesHandler.KEY_MAIL_SUBJECT_ID);
