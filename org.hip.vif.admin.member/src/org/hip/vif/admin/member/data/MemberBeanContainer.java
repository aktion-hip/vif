 ...
 *
 * @author Luthiger Created: 15.11.2011 */
...
public class MemberBeanContainer extends BeanItemContainer<MemberBean> {
     ...
     * @throws VException
     * @throws SQLException */
    public static MemberBeanContainer createData(final QueryResult inMembers) throws VException, SQLException {
        final MemberBeanContainer outContainer = new MemberBeanContainer();
    ...
    /** The <code>Select</code> widget displays already selected member entries only if they are contained in the item
     * container bound to the widget. Therefore, we add the selected entries here.
     *
     * @param inSelected {@link Collection} of <code>MemberBean</code>s */
    public void addSelected(final Collection<MemberBean> inSelected) {
        for (final MemberBean lMember : inSelected) {
            addItem(lMember);
        }
    }
