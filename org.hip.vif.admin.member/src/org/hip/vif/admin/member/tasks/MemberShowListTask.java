...
public class MemberShowListTask extends AbstractMemberSearchTask {
    ...
    protected Component runChecked() throws RiplaException {
        try {
            ...
            return new MemberListView(createMemberContainer(QueryHelper.getQueryStrategy(getParameters())),
                    !isExternal(), this);
        } catch (final NoHitsException exc) {
            Notification
                    .show(
                            Activator.getMessages().getFormattedMessage("errmsg.search.no.hits", exc.getQueryString()), Type.WARNING_MESSAGE); //$NON-NLS-1$
