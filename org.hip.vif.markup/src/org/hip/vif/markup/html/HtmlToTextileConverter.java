...
public class HtmlToTextileConverter extends DefaultHandler {
    ...
    public String convertToTextile(final String inHTML) {
        final StringReader lReader = new StringReader(
                String.format(XML, TAG_ROOT, normalizeSpace(makeTidy(inHTML)), TAG_ROOT));
    ...
    private String makeTidy(final String inHTML) {
    ...
    private String normalizeSpace(final String inText) {
        return WHITE_SPACE.matcher(inText.replace(NL_TAG, "<")).replaceAll(" ");
    }

    private String normalizeNL(final String inText) {
        final String out = MULTI_LINE.matcher(inText).replaceAll(NL + NL);
    ...
    @Override
    public void startElement(final String inUri, final String inLocalName, final String inQName,
        ...
        final IHandlerFactory lFactory = tagFactories.get(inQName.toLowerCase());
        actHandler = lFactory == null ? DFT_FACTORY.createHandler(inAttributes) : lFactory.createHandler(inAttributes);
    }

    @Override
    public void endElement(final String inUri, final String inLocalName, final String inQName) throws SAXException {
        final StringBuilder lPart = actHandler.endTag();
    ...
    private static abstract class TagHandler {
        ...
        public void append(final String inText) {
            content.append(inText);
        }
    }

    private static class QuickTag extends TagHandler implements ITagHandler {
        private final String wrapChar;
        private final String tag;

        QuickTag(final String inTag, final String inWrapChar) {
            tag = inTag;
            wrapChar = inWrapChar;
    ...
    private static class Block extends TagHandler implements ITagHandler {
        ...
        Block(final String inHtmlTag, final String inTag) {
            htmlTag = inHtmlTag;
    ...
    private static class List extends TagHandler implements ITagHandler {
        ...
        public StringBuilder endTag() {
            ...
            final String outContent = new String(content).substring(0, content.length() - NL.length());
    ...
    private static class EndTagger extends TagHandler implements ITagHandler {
        ...
        EndTagger(final String inEndTag) {
            endTag = inEndTag;
    ...
    private static class LinkHandler extends TagHandler implements ITagHandler {
        ...
        LinkHandler(final String inHref) {
            href = inHref;
            content = new StringBuilder(" \"");
        }
    ...
    private static class ImageHandler extends TagHandler implements ITagHandler {
        ImageHandler(final String inSrc) {
    ...
    private static class QuickTagFactory implements IHandlerFactory {
        ...
        QuickTagFactory(final String inTag, final String inWrapChar) {
            tag = inTag;
            wrapChar = inWrapChar;
        }

        @Override
        public ITagHandler createHandler(final Attributes inAttributes) {
    ...
    private static class BlockFactory implements IHandlerFactory {
        ...
        BlockFactory(final String inHtmlTag, final String inTag) {
            htmlTag = inHtmlTag;
            tag = inTag;
        }
    ...
    private static class ListFactory implements IHandlerFactory {
        ...
        ListFactory(final String inTag, final String inPrefixSymbol) {
            tag = inTag;
    ...
    private static class EndTagFactory implements IHandlerFactory {
        ...
        EndTagFactory(final String inEndTag) {
            endTag = inEndTag;
        }
    ...
    private static class DivFactory implements IHandlerFactory {
        @Override
        public ITagHandler createHandler(final Attributes inAttributes) {
            final String lStyle = inAttributes.getValue("style");
            if (lStyle == null) {
                return new Block("normal", "p");
            }
