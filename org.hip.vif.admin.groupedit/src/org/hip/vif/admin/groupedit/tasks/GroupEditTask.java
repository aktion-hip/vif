...
public class GroupEditTask extends AbstractGroupTask implements
	...
	protected Component runChecked() throws VException {
		...
		try {
			...
			final Group lGroup = BOMHelper.getGroupHome().getGroup(lGroupID);
		...
		try {
			...
			showNotification(
					Activator.getMessages().getMessage(
							"admin.group.edit.data.saved"), Type.TRAY_NOTIFICATION); //$NON-NLS-1$
	...
	public boolean groupClose() {
		try {
			final Group lGroup = BOMHelper.getGroupHome()
					.getGroup(getGroupID());
			return doTransition(lGroup.getCloseTransition());
		}
		catch (final VException exc) {
			LOG.error(
					"Error encountered during a group's state transition!", exc); //$NON-NLS-1$
		}
		return false;
	}
	...
	private boolean doTransition(final String inTransition) {
		try {
			...
			if (lStateChange.doNotification()) {
				...
				showNotification(
						Activator.getMessages().getMessage(
								"admin.group.state.change"), Type.TRAY_NOTIFICATION); //$NON-NLS-1$
	...
	public boolean selectAdmin() {
		try {
			...
			LOG.error(
					"Error encountered while selecting the group administration!", exc); //$NON-NLS-1$
		}
		return false;
	}
	...
	public boolean selectParticipants() {
		try {
			...
			lParameters.set(Constants.KEY_PARAMETER_TITLE_SEARCH, lMessages
					.getFormattedMessage(
							"ui.group.lookup.search2.title.page", lGroupName)); //$NON-NLS-1$
			lParameters.set(Constants.KEY_PARAMETER_SUBTITLE_SELECT, lMessages
					.getFormattedMessage(
							"ui.group.lookup.sub2.title.page", lGroupName)); //$NON-NLS-1$
			lParameters
					.set(Constants.KEY_PARAMETER_RIGHT_COLUMN,
							lMessages
									.getFormattedMessage(
											"ui.group.lookup.selected.participants", lGroupName)); //$NON-NLS-1$
			lParameters.set(Constants.KEY_PARAMETER_SELECTED,
					new Vector<ParticipantBean>());
			lParameters.set(Constants.KEY_PARAMETER_PROCESSING,
					createFullyQualifiedTaskName(ParticipantSelectTask.class));
			setParameters(lParameters);
			requestLookup(LinkButtonHelper.LookupType.MEMBER_SEARCH, 0l);
			return true;
		}
			...
			LOG.error(
					"Error encountered while selecting the group participants!", exc); //$NON-NLS-1$
		}
		return false;
	}
	...
	public boolean deleteAdmin() {
		try {
			...
			LOG.error(
					"Error encountered while deleting the group administration!", exc); //$NON-NLS-1$
		}
		catch (final SQLException exc) {
			LOG.error(
					"Error encountered while deleting the group administration!", exc); //$NON-NLS-1$
