 ...
 * A bitmap represents a matrix of binary values.
 * 
 * @author Benno Luthiger
 */
abstract public class AbstractBitMap extends VObject {
	//constants
	 ...
	 */
	protected AbstractBitMap(CodeList inColumns, QueryResult inRows, QueryResult inContent) throws VException {
		super();
		columnIDs = loadColumnIDs(inColumns);
		bitmap = initBitMap(inRows);
		bitmap = loadBitMap(bitmap, inContent);
	}
	 ...
	 */
	protected AbstractBitMap(String[] inColumnIDs, BitField inBitField) {
		super();
		columnIDs = inColumnIDs;
		bitmap = inBitField;
	}
	...
	/**
	 * Hoof for initializing the bit field, i.e. loading the rows.
	 * Note: Initialization has to be separated from loading, because
	 * the loading of the content requires an initialized bit field.
	 * 
	 * @param inRows org.hip.kernel.bom.QueryResult
	 ...
	 * @return boolean
	 */
	public boolean checkElementAt(int inRow, int inColumn) {
	 ...
	 * @return int
	 */
	protected int rowIndexOf(String inRowID) {
	 ...
	 * @return int
	 */
	protected int columnIndexOf(String inColumnID) {
	 ...
	 */
	protected void addNewRow(Object inObject) {
	...
	public String toString() {
		StringBuffer outXML = new StringBuffer("<"+TAG_ROOT+">\n");
		
		for (int i = 0; i < bitmap.getRowSize(); i++) {
			Tuple lTuple = bitmap.getTuple(i);
			outXML.append("\t<"+TAG_ROW + " " + ATTRIBUTE_ROW + "='" + getRowID(lTuple.getTupleObject()) + "'>\n");
			
			for (int j = 0; j < columnIDs.length; j++) {
				outXML.append("\t\t<"+TAG_COLUMN + " " + ATTRIBUTE_COLUMN + "='" + columnIDs[i] + "'>");
				outXML.append(checkElementAt(i, i) ? "1" : "0");
				outXML.append("</"+TAG_COLUMN+">\n"); 
			} //for
			
			outXML.append("</"+TAG_ROW+">\n");
		} //for
		
		outXML = new StringBuffer("</"+TAG_ROOT+">");
		
		return new String(outXML);
	}
	
	/**
	 * Returns a string representation of this bit pattern in
	 * form of an XML.
	 * 
	 * @param java.lang.String
	 */
	public String toXML() {
		...
		for (int i = 0; i < bitmap.getRowSize(); i++) {
			outXML.append("\t<"+TAG_ROW + ">\n");
			Tuple lTuple = bitmap.getTuple(i);
			String lRowID = getRowID(lTuple.getTupleObject());
			outXML.append(getRowAddToXML(lTuple.getTupleObject()));
			
			for (int j = 0; j < columnIDs.length; j++) {
				outXML.append("\t\t<"+ TAG_ELEMENT + 
					" " + ATTRIBUTE_ROW + "='" + lRowID + "' " +
					" " + ATTRIBUTE_COLUMN + "='" + columnIDs[j] + "' " +
					" " + ATTRIBUTE_CHECKED + "='" + (checkElementAt(i, j) ? ATTRIBUTE_CHECKED : "") + "'>");
				outXML.append("</" + TAG_ELEMENT + ">\n"); 
			} //for
	 ...
	 * @return java.lang.String
	 */
	protected String getRowID(int inRowPosition) {	
	 ...
	 */
	protected Tuple remove(int inRowPosition) {
	 ...
	 */
	public IDPositions getCheckedOfRow(String inRowID) {
		IDPositions outPositions = new IDPositions();
		int lRowPosition = rowIndexOf(inRowID);
		for (Iterator<?> lPositions = bitmap.getCheckedOfRow(lRowPosition).iterator(); lPositions.hasNext();) 
	 ...
	 */
	public Collection<IDPosition> getCheckedOfRow2(String inRowID) {
		Collection<IDPosition> outPositions = new Vector<IDPosition>();
		int lRowPosition = rowIndexOf(inRowID);
		for (MatrixPosition lPosition : bitmap.getCheckedOfRow2(lRowPosition)) {
	 ...
	 */
	public IDPositions convertPositions(MatrixPositions inMatrixPositions) {
		IDPositions outPositions = new IDPositions();
	 ...
	 * @return Collection<IDPosition>
	 */
	public Collection<IDPosition> convertPositions(Collection<MatrixPosition> inMatrixPositions) {
		Collection<IDPosition> outPositions = new Vector<IDPosition>();
		for (MatrixPosition lPosition : inMatrixPositions) {
			outPositions.add(new IDPosition(getRowID(lPosition.getRowPosition()), columnIDs[lPosition.getColumnPosition()]));
		}
		return outPositions;
	}
