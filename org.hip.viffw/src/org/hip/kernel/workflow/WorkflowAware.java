 ...
 * implemented as methods of the class implementing this interface.<br/>
 * These methods get associated with the individual
 ...
 * 
 *   1. if implemented, the method <code>onLeave_Private</code> is called
 *      (it is called each time the object leaves the 'Private' state)
 *
 *   2. if implemented, the method <code>onTransition_Publish</code> is called
 *      (it is called whenever this transition is executed)
 *
 *   3. if implemented, the method <code>onEnter_Public</code> is called
 *      (it is called each time the object enters the 'Public' state)
 * 
 * @author Benno Luthiger
 * (inspired by itools by Juan David Ibanez Palomar <jdavid@itaapy.com>)
 */
