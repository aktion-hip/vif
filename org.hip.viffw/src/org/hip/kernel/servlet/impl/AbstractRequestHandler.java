...
/**
 * 	Base class of all request handler. All servlet requests are handled from this
 *  request handler. The request type-parameter is required for every request. It holds
 *  the name of the task to run. Application specific request handler which subclass this
 *  base class, have to specify an taskmanager.
 *
...
public abstract class AbstractRequestHandler extends HttpServlet implements RequestHandler {
	...
	/**
	 * Constructor of AbstractRequestHandler.
	...
	/**
	 * Checks various parameters of the request and, if successful, handles
	 * it, i.e. executes the requested task.
	 *
	 ...
	 */
	private void checkAndHandleRequest(HttpServletRequest inRequest, HttpServletResponse inResponse, Context inContext) throws ServletException, IOException, RequestException {
		try {		
			ServletRequestHelper lHelper = new ServletRequestHelper(inRequest);
			
			//check if the requestType is defined, if not, we run the default task
			String lRequestType = lHelper.containsParameter(AbstractContext.REQUEST_TYPE) ? lHelper.getParameterValue(AbstractContext.REQUEST_TYPE) : getDefaultRequestType();
			...
			if ("memoryInfo".equals(lRequestType)) {
				showMemoryInfo(inContext, inResponse);
				return;
			}
			...
			Task lTask = taskManager.create(lRequestType);
	...
	/**
	 * We set the default task to be displayed in the body frame if nothing is specified.
	 * Subclasses may override and provide a different request type for the startup.
	 * 
	 * @param inContext Context
	 */
	protected void setStartupRequestType(Context inContext) {
		inContext.setParameter(BODY_PARAMETER, AbstractTaskManager.DEFAULT_TASK_NAME);		
	}

	/**
	 * By default, we run the master task and set the default task to be displayed in the body frame.
	 * Subclasses may override and provide a different request type or throw a <code>RequestException</code>.
	 * 
	 * @param inContext Context
	...
	public void doGet(HttpServletRequest inRequest, HttpServletResponse inResponse) throws ServletException, IOException {
		...
		Context lContext = this.getContext(inRequest);
	 ...
	 * @throws RequestException 
	 */
	protected void writeToResponse(HttpServletResponse inResponse, Context inContext, String inSessionID) throws RequestException, IOException {
		HtmlView lView = inContext.getView();
	 ...
	 * @exception java.io.IOException
	 */
	public void doGetMIME(HttpServletRequest inRequest, HttpServletResponse inResponse) throws ServletException, IOException {
		try {
	...
	/**
	 * Overrides the super class's init() method.
	 * Used to set application specific parameters to VSys.
	 * 
	...
	/**
	 * Subclasses have to provide the name of the application's properties file,
	 * e.g. <code>vif</code> for <code>vif.properties</code>
	 * 
	...
	/**
	 * This method starts an ErrorTask to send an error message to the client.
	 * The parameter inMessage will be displayed in this view.
	 *
	 * @param inResponse javax.servlet.http.HttpServletResponse
	 * @param inMessage java.lang.String
	 */
	public void error(HttpServletRequest inRequest, HttpServletResponse inResponse, String inMessage) {
		try {
			Task lTask = taskManager.getErrorTask();
			Context lContext = this.getContext(inRequest);
			...
			HtmlView lView = (HtmlView)lContext.get(AbstractContext.VIEW);
	...
	/**
	 * Returns the context object out of the actual session.
	 * If the session is just created a new context object
	 * will be created and added to the session object.
	 *
	 * @return org.hip.kernel.servlets.Context
	 */
	protected Context getContext(HttpServletRequest inRequest) {
		HttpSession lSession = inRequest.getSession(true);
		Context outContext = null;
		boolean lNewSession = lSession.isNew();
		...
		outContext.set(AbstractContext.NEW_SESSION_KEY, new Boolean(lNewSession));
	...
	/**
	 * Subclasses which implement this method, have to return an instance 
	...
	private Context loadContextObject() {
		...
		try {
			...
			String lContextClassName = this.getContextClassName();
			Class<?> lContextClass = Class.forName(lContextClassName);
	 ...
	 * @param inResponse javax.servlet.http.HttpServletResponse
	 */
	private void showMemoryInfo(Context inContext, HttpServletResponse inResponse) throws Exception {
		HtmlView lView = new AbstractHtmlView() {
			public void renderToStream(ServletOutputStream inStream, String inSessionID) {
				try {
			...
			public void renderToWriter(PrintWriter inWriter, String inSessionID) throws RequestException {
				final int MEGA_BYTES = 1048576;
				inWriter.println("<h2>Memory</h2>");
				inWriter.println("<b>Total memory:</b> " + Runtime.getRuntime().totalMemory() / MEGA_BYTES + " MB <br/>");
	...
	/**
	 * Hook for subclasses.
	 * This method can be overwritten in subclasses to check for request types
	 * which have to be processed in a special way, i.e. by an implementation
	 * of subRequestTypeHandler()
	 * 
	...
	/**
	 * Hook for subclasses.
	 * This method can be overwritten in subclasses to process a request type 
	 * in a special way.
	 * 
	 * @param inContext org.hip.kernel.servlet.Context
	 * @param inResponse javax.servlet.http.HttpServletResponse
	 ...
	 * @return String
	 */
	private String getSessionID(HttpServletRequest inRequest, Context inContext) {
		String outSessionID = "";
