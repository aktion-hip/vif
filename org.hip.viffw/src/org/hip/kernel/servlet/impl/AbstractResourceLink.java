...
/**
 * This class provides generic functionality for external resources linked to
 * the html file.
 * 
...
public abstract class AbstractResourceLink extends VObject implements Serializable {
	...
	protected static String WEB_CSS_ROOT = "";
	 ...
	 */
	public AbstractResourceLink(String inHref) {
		super();
		setHref(inHref);
	}
	 ...
	 */
	public AbstractResourceLink(String inHref, String inMedia) {
		this(inHref);
		setMedia(inMedia);
	}
	
	/**
	 * Set the relative path to the css file. (Path has to be relative to the
	 * css rootdirectory path! Defined in vif.properties).
	 * 
	...
	public void setHref(String inHref) {
		...
		try {
			...
			throw new VError("AbstractResourceLink.initialize(): error during getting css root properties" );
		}	
	}
	...
	public boolean equals(Object inObject) {
		...
		if (!(inObject instanceof AbstractResourceLink)) return false;
	
		AbstractResourceLink lLink = (AbstractResourceLink)inObject;
	 ...
	 * 
	 * ex: <data>screen</data> whend the stylesheet is only for outputs on the screen
	 *	   <data>print</data> for output on a printer
	 *     <data>all</data>	  for every outputmedia
	 *
	 * There are more, like tv, handheld, ... but even the ones above are not supported in all version of 
	 * the IE 4.0 or Netscape 4.0.
	 * 
	 * @param inMedia java.lang.String
	 */
	public void setMedia(String inMedia) {
