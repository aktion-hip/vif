...
/**
 * Adapter to create the filters used to retrieve items from a LDAP store.
 * This class is used by <code>LDAPObjectHome</code>.
 *
 * @author Luthiger
...
public class LDAPAdapter {
	 ...
	 */
	public String createSelectString(KeyObject inKey, GeneralDomainObjectHome inDomainObjectHome) {
		return createFilter(inKey, inDomainObjectHome);
	}

	private String createFilter(KeyObject inKey, final GeneralDomainObjectHome inDomainObjectHome) {
		// Pre: inKey not null
		if (inKey == null)
			return "";
		...
		StringBuffer outFilters = new StringBuffer("(");
		outFilters.append(inKey.render(inDomainObjectHome)).append(")");
