...
public class NestedDefImpl extends AbstractDefWithColumns implements NestedDef {
	 ...
	 */
	public NestedDefImpl(String inName) {
		super();
		try {
	 ...
	 */
	public void addGroupingDef(GroupingDef inGroupingDef) {
		try {
	...
	public String getNestedQuery() {
		String outSQL = "(SELECT " + getFields() + " ";
		return outSQL + getFromSQL() + ") " + getName();
	}
	
	private String getFromSQL() {
		StringBuffer outSQL = new StringBuffer("FROM ");
		outSQL.append(getTableName());
		String lGrouping = getResultGrouping();
	...
	private String getResultGrouping() {
		String outGrouping = "";
		try {
			GroupingDef lGroupingDef = (GroupingDef)get(NestedDefDef.resultGrouping);
	...
	/**
	 * Returns the SQL name of the column mapped to the <columnDef> entry
	 * with the specified columnName attribute.
	 * 
	 * @param inColumnName String
	 * @return String
	 */
	public String getSQLColumnName(String inColumnName) {
	...
	public boolean equals(Object inObject) {
		...
		if (!(inObject instanceof NestedDef)) return false;
		
		NestedDef lNestedDef = (NestedDef)inObject;
