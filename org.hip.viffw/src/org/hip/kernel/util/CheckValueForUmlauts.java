...
/**
 * This class is a utility class needed to construct sql strings
 * for a select with a LIKE statement.
 * The search has to be tolerant against inputed umlauts.
 *
 * @author	Benno Luthiger
 */
public class CheckValueForUmlauts {
	private Vector<String> searchStrings;
	...
	public CheckValueForUmlauts(String inSearchString) {
		...
		inSearchString = inSearchString.toUpperCase();
		FindFirstUmlaut lFindUmlaut = new FindFirstUmlaut(inSearchString);
		if (lFindUmlaut.hasUmlauts()) {
			String[] lSearchStrings =  {""};
			lSearchStrings = processUmlauts(inSearchString, lSearchStrings);
			searchStrings = new Vector<String>(lSearchStrings.length);
			for (int i = 0; i < lSearchStrings.length; i++) {
				searchStrings.addElement(lSearchStrings[i]);
			}
		}
		else {
			searchStrings = new Vector<String>(1);
			searchStrings.addElement(inSearchString);
		}
	}
	 ...
	 */
	public synchronized Iterator<String> elements() {
		return searchStrings.iterator();		
	}
	
	/**
	 * Recursive function!
	 * Returns an array of Strings containing all variation of umlauts
	 ...
	 */
	private String[] processUmlauts(String inRemaining, String[] inSearchStrings) {
		String[] outSearchStrings;
	
		FindFirstUmlaut lFindUmlaut = new FindFirstUmlaut(inRemaining);
		if (lFindUmlaut.hasUmlauts()) {
			...
			int lOffset = inSearchStrings.length;
			for (int i = 0; i < inSearchStrings.length; i++) {
