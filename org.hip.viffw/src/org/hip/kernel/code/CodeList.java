...
/**
 * A CodeList consists of various items associating a code with a 
...
public class CodeList {
	//instance attributes
  	private Vector<CodeListItem>     codeListElements = new Vector<CodeListItem>();
  	private Hashtable<String, String>  codeLabelsKeyValue;
  	private TreeSet<String> 	codeLabels;
  	private String[]   codeLabelsUnsorted;
  	private String[]   codeElementID;
  	private String[]   codeElementIDUnsorted;
	...
	/**
	 * CodeList constructor, should only be called by CodeListHome.
	 * Creates an instance of CodeList with the specified codeID for
	 * the specified language.
	 */
	CodeList(String inCodeID, String inLanguage) {
		codeID   = inCodeID;
	  	language = inLanguage;
	 ...
	 */
	void add(CodeListItem inElement) {
	  	codeListElements.addElement(inElement);
	}
	/**
	 ...
	 */
	public boolean existElementID(String inElementID) throws CodeNotFoundException {   
		Object lItem = codeLabelsKeyValue.get(inElementID);
	 ...
	 * @exception CodeNotFoundException: If no appropriate ElementID can be found
	 */
	public String getElementIDByLabel(String inLabel) throws CodeNotFoundException {
		Iterator<CodeListItem> lItems = getElementIdCollection().iterator();
		while (lItems.hasNext()) {
			CodeListItem lItem = lItems.next();
	 	...
	 	lItems = getElementIdCollection().iterator();
	  	while(lItems.hasNext()) {
			...
			StringTokenizer lElementLabelTokens = new StringTokenizer(lItem.getLabel());
			StringTokenizer lInputLabelTokens 	= new StringTokenizer(inLabel);  // must be new every time !!!
	 ...
	 **/
	public Vector<CodeListItem> getElementIdCollection(){
	  	return codeListElements;
	}
	/**
	 * Get a sorted array of all short labels (ElementIDs) to this codeID
	 * The order of the ElementIDs is in a way that the associated
	 * labels appear in alphabetically sorted order.
	 *
	 * @return java.lang.String[] the sorted array of short labels
	 */
	public String[] getElementIDs() {
	 	String[] outIDs = new String[codeElementID.length];
	...
	public String[] getElementIDsUnsorted() {	
	  	String[] outIDs = new String[codeElementIDUnsorted.length];
	 ...
	 * @param strElementID
	 */
	public String getLabel(String inElementID) {
	...
	public String[] getLabels() {
		String[] outLabels = new String[codeLabels.size()];
		int i = 0;
		for (Iterator<String> lLabels = codeLabels.iterator(); lLabels.hasNext();) {
	...
	public String[] getLabelsUnsorted() {
	  	String[] outLabels = new String[codeLabelsUnsorted.length];
	 ...
	 * @return boolean
	 */
	private boolean isSelected(String inElementID, AbstractCode[] inSelectedCodes) {
	 ...
	 * @return boolean
	 */
	private boolean isSelected(String inElementID, Collection<? extends AbstractCode> inSelectedCodes) {
		for (AbstractCode lCode : inSelectedCodes) {
	...
	void prepareArrays() {
		...
		try{
		  	...
		  	int i = 0;
		  	for (CodeListItem lItem : codeListElements) {
	 ...
	 */
	private boolean tokensAreEqual(StringTokenizer inTokenizer, StringTokenizer inTokenizerToCompare) {
	
	  	if (inTokenizer.countTokens() != inTokenizerToCompare.countTokens())
			return false;
	
		while (inTokenizer.hasMoreTokens()) {
			String lToken = inTokenizer.nextToken();
			String lTokenToCompare = inTokenizerToCompare.nextToken();
	...
	public String toLongString() {
		ListJoiner outLabels = new ListJoiner();
		for (String lLabel : codeLabels) {
	 ...
	 *	<codeList> 
	 *		<codeListItem id="id1" >Item1</codeListItem> 
	 *		<codeListItem id="id2" selected="true">Item2</codeListItem> 
	 *		<codeListItem id="id3" >Item3</codeListItem> 
	 *		<codeListItem id="id4" selected="true">Item4</codeListItem>
	 *			...
	 *	</codeList> 
	 *
	 * @param inSelectedCodes org.hip.kernel.code.AbstractCode[] Array of Codes which are selected
	 ...
	 */
	public String toSelectionString(AbstractCode[] inSelectedCodes) {
	 ...
	 * @return java.lang.String XML
	 */
	public String toSelectionString(Collection<? extends AbstractCode> inSelectedCodes) {
	  	StringBuffer outXML = new StringBuffer();
	...
	public String toString() {
		ListJoiner outIDs = new ListJoiner();
		for (String lID : codeElementID) {
			outIDs.addEntry(lID);
		}
		return outIDs.join(";");
	}
