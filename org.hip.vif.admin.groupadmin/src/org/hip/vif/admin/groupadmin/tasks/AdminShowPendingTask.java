...
/** Shows the list of pending tasks of a group administrator, e.g. the list of questions in the group where a state
    ...
    protected Component runChecked() throws RiplaException {
        try {
            ...
            if (lCountAnswerables + lCountReopen == 0) {
                return reDisplay(
                        Activator.getMessages().getMessage("admin.msg.pending.none"), Notification.Type.HUMANIZED_MESSAGE); //$NON-NLS-1$
            }
    ...
    private abstract class Processor {
        ...
        boolean process(final String inMsgKey) {
            try {
                ...
                LOG.error("Error encountered while setting question to state 'answered'!", exc); //$NON-NLS-1$
            } catch (final VException exc) {
                LOG.error("Error encountered while setting question to state 'answered'!", exc); //$NON-NLS-1$
            } catch (final SQLException exc) {
                LOG.error("Error encountered while setting question to state 'answered'!", exc); //$NON-NLS-1$
