...
/** Task to delete bibliography entries by displaying the deltable, referenced and undeletable entries.
 *
...
public class BibliographyDeleteTask extends AbstractBibliographyTask implements
    ...
    private ContributionContainer retrieveTexts(final Collection<Long> inTexts,
        ...
        lKey.setValue(TextHome.KEY_STATE, WorkflowAwareContribution.S_OPEN, "=", BinaryBooleanOperator.AND); //$NON-NLS-1$
        ...
        try {
            ...
            if (lEntry instanceof ContributionWrapper) {
                ...
                if (referenced.getItemIds().contains(lText)) {
                    ...
                    UI.getCurrent().addWindow(lLookup.getLookupWindow());
                } else {
     ...
     * 
     * @param inReferenced {@link ContributionContainer}
    ...
    public boolean removeReferences(final ContributionContainer inReferenced) {
        try {
            ...
            showNotification(Activator.getMessages().getMessage("msg.bibliography.link.removedP")); //$NON-NLS-1$
     ...
     * 
     * @param inDeletable {@link ContributionContainer}
     * @return boolean <code>True</code> if successful */
