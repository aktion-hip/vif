...
/**
 * Task to search for a Bibliography entry for that it can be linked to a
 * question.
 * 
...
public class BibliographyHandleTask extends AbstractBibliographyTask implements
	 ...
	 * 
	 * @param inTitle
	 *            Object, input, may be <code>null</code>
	 * @param inAuthor
	 *            Object, input, may be <code>null</code>
	 * @return boolean <code>true</code> if the input has been processed
	 *         successfully, <code>false</code> else.
	 */
	...
	/**
	 * Returns the data container containing the text entries according to the
	 * input in the combo boxes.
	 * 
	...
	public void addBibliography() {
		...
		try {
			...
			LOG.error(
					"Error while linking the bibliography item to the question.", exc); //$NON-NLS-1$
		}
		catch (final SQLException exc) {
			LOG.error(
					"Error while linking the bibliography item to the question.", exc); //$NON-NLS-1$
	...
	/**
	 * Method called when user clicks the table of the search results => show
	 * bibliography entry in popup window.
	 * 
	 ...
	 * If more the one entries are selected, the first of the is displayed in
	 * edit view.
	 */
	 ...
	 * The application only deletes entries if they're not references.
	 * Therefore, the actor might have to remove references first for that entry
