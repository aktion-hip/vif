...
public class VIFMemberIndexerTest {
	...
	public void testRefreshMemberInIndex() throws Exception {
		String lMail1 = "test@my.site.org";
		String lMail2 = "changed@my.site.org";

		assertEquals("number of indexed 1", 2, IndexHouseKeeper.countIndexedMembers());
		
		String lID = data.createMember("91", lMail1);
		assertEquals("number of indexed 2", 3, IndexHouseKeeper.countIndexedMembers());
		...
		Member lMember = data.getMemberHome().getMember(lID);
	...
	private Document[] searchMail(String inMail) throws CorruptIndexException, NoHitsException, IOException {
		return IndexHouseKeeper.search(new TermQuery(new Term(AbstractSearching.IndexField.MEMBER_MAIL.fieldName, inMail)), reader);
	}
	
	private IndexReader openReader(IndexReader inReader) throws Exception {
		if (inReader != null) {
			inReader.close();
		}
		return IndexReader.open(IndexHouseKeeper.getMembersIndexDir());
	}
