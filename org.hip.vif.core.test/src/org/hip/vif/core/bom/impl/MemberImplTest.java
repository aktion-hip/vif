...
public class MemberImplTest {
	...
	public void testNew() throws Exception {
		...
		String[] lExpectedRoles = new String[] {"1", "3"};

		//pre
		data.checkForEmptyTable();
		
		Member lMember = (Member)lHomeMember.create();
		lMember.ucNew("userT1", "NameT", "VornameT", "StrasseT", "PLZ-T", "StadtT", "", "", "mail@test", "1", "de", "123", lExpectedRoles);
	...
	public void testSave() throws Exception {
		String lNamePre = "Befor";
		String lNamePost = "After";
		
		//pre
		data.checkForEmptyTable();
		
		String lMemberID = String.valueOf(data.createMember2Roles(lNamePre));
		MemberHome lMemberHome = new MemberHomeImplSub();
		
		Member lMember = lMemberHome.getMember(lMemberID);
		assertEquals("Member name 1", lNamePre, (String)lMember.get(MemberHome.KEY_NAME));
		
		MemberHistoryHome lHistory = data.getMemberHistoryHome();
		int lHistoryCount = lHistory.getCount();
		
		//save with new name
		boolean lChangedRoles = lMember.ucSave(lNamePost, "FiName", "Street", "PLZ", "City", "", "", "test@mail", "1", "de", new String[] {"1", "3"}, new Long(21));
		...
		assertEquals("Member name 2", lNamePost, (String)lMember.get(MemberHome.KEY_NAME));

		//save with new roles
		lChangedRoles = lMember.ucSave(lNamePost, "FiName", "Street", "PLZ", "City", "", "", "test@mail", "1", "de", new String[] {"2", "4"}, new Long(21));
	...
	private void waitSomeMillies(int inMillies) {
		long lTo = System.currentTimeMillis() + inMillies;
	...
	public void testSave2() throws Exception {
		...
		Member lMember = lMemberHome.getMember(lMemberID);
		assertEquals(lNamePre, (String)lMember.get(MemberHome.KEY_NAME));
		...
		lMember = lMemberHome.getMember(lMemberID);
		assertEquals(lNamePost, (String)lMember.get(MemberHome.KEY_NAME));
	...
	private Collection<String> getRoles(String[] inRoles) {
	...
	public void testDelete() throws Exception {
		...
		String lMemberID = String.valueOf(data.createMember2Roles());
		
		MemberHome lMemberHome = data.getMemberHome();
		LinkMemberRoleHome lLinkHome = data.getLinkMemberRoleHome();
		...
		catch (org.hip.kernel.sys.AssertionFailedError exc) {
			//intentionally left empty
		}
		try {
			lMember.delete();
	...
	public void testCreateSU() throws Exception {
		...
		MemberHome lMemberHome = data.getMemberHome();
		Member lMember = (Member)lMemberHome.create();
		...
		Collection<Role> lRoles = lLinkHome.getRolesOf(new Long(lMemberHome.select().next().get("ID").toString()));
		for (Role lRole : lRoles) {
	...
	public void testGetBestRole() throws Exception {
		...
		int lCountMembers = lHome.getCount();
		int lCountLink = data.getLinkMemberRoleHome().getCount();
		Long lMemberID = data.createMemberRoles("TestMember", new String[] {ApplicationConstants.ROLE_ID_SU, ApplicationConstants.ROLE_ID_GROUP_ADMIN});
		...
		assertEquals("Best role 1", Integer.parseInt(ApplicationConstants.ROLE_ID_SU), ((MemberHome)lHome).getMember(lMemberID.toString()).getBestRole());
		
		data.deleteAllFromMember();
		data.deleteAllFromLinkMemberRole();
		lMemberID = data.createMemberRoles("TestMember2", new String[] {ApplicationConstants.ROLE_ID_MEMBER, ApplicationConstants.ROLE_ID_GROUP_ADMIN, ApplicationConstants.ROLE_ID_PARTICIPANT});
		assertEquals("Count member 2", 1, lHome.getCount());
		assertEquals("Count link 2", 3, data.getLinkMemberRoleHome().getCount());
		assertEquals("Best role 2", Integer.parseInt(ApplicationConstants.ROLE_ID_GROUP_ADMIN), ((MemberHome)lHome).getMember(lMemberID.toString()).getBestRole());
	}
	
	@Test
	public void testIndexing() throws Exception {
		String lUserName = "TestIndex";
		String lUserID = "userTest";
		...
		lMember.ucNew(lUserID, lUserName, "VornameT", "StrasseT", "PLZ-T", "StadtT", "", "", "mail@test", "1", "de", "123", new String[] {"1"});
		...
		lMember.ucSave(lUserName, "VornameT2", "StrasseT2", "PLZ-T2", "StadtT2", "", "", "mail@test", "1", "de", new Long(44));
	...
	private Document[] search(String inQuery) throws CorruptIndexException, IOException, NoHitsException, ParseException {
...
//	--- private classes ---
	...
	/**
	 * We need to creat special member objects for testing purposes, see <code>org.hip.vif.bom.impl.test.TestMemberImpl</code>.
	 */
