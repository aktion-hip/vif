...
public class IndexHouseKeeper {
	...
	private static File createDirStructure(String inDirName) {
		File outDir = new File(inDirName);
		if (!outDir.exists()) {
			outDir.mkdirs();
		}
		return outDir;
	...
	/**
	 * Delete the lucene index dirs created for testing purpose.
	 * Should be called during <code>TestCase.tearDown()</code>.
	 * 
	 * <b>Note:</b> (17.12.2010) we only close and don't delete. Deleting during the test run results in corrupt indexes and stale file locks.
	 * @throws IOException
	 */
...
//}
	 ...
	 * 
	 * @param inBothIndexes boolean if <code>true</code> both member and content indexes are created, if <code>false</code>, only the member index is created
	 * @throws Exception
	 */
	public static void redirectDocRoot(boolean inBothIndexes) throws Exception {
	...
	private static void createTestIndex(String inIndexName) throws CorruptIndexException, LockObtainFailedException, IOException {
		IndexWriterConfig lConfig = new IndexWriterConfig(LUCENE_VERSION, getAnalyzer());
		lConfig.setOpenMode(OpenMode.CREATE);
		Directory lDirectory = getDirectory(inIndexName);
		IndexWriter lWriter = new IndexWriter(lDirectory, lConfig);
		try {
			lWriter.close();
		}
		finally {
	...
	private static Directory getDirectory(String inIndexName) throws IOException {
		return FSDirectory.open(checkIndexDir(createPath(inIndexName)));
	}	
	
	private static String createPath(String inName) {
		return WorkspaceHelper.getRootDir() + File.separator + INDEX_DIR + inName;
	}
	
	private static File checkIndexDir(String inPath) {
	...
	/**
	 * <p>Searches the index for the specified <code>Query</code>.</p>
	 * <p>You can evaluate the returned <code>Hits</code> object as follows:</p>
	 * <pre>Document[].length</pre>
	 * 
	 * @param inQuery Query as it is returned by <code>QueryParser.parse(String query, String field, Analyzer analyzer)</code>
	 * @return Document[]
	 * @throws IOException 
	 * @throws CorruptIndexException 
	 * @throws NoHitsException 
	 */
	public static Document[] search(Query inQuery) throws CorruptIndexException, IOException, NoHitsException {
		Document[] outHits;
		IndexReader lReader = IndexReader.open(getMembersIndexDir());
		try {
	...
	/**
	 * <p>Searches the index for the specified <code>Query</code> using the specified <code>IndexReader</code>.</p>
	 * <p>You can evaluate the returned <code>Hits</code> object as follows:</p>
	 * <pre> Document[].length
	 * Document[0].get(AbstractSearching.MEMBER_ID)</pre> 
	 * <p><b>Note:</b> you have to close the <code>IndexReader</code> you pass by yourself.</p> 
	 * 
	 * @param inQuery Query
	 ...
	 * @throws CorruptIndexException 
	 */
	public static Document[] search(Query inQuery, IndexReader inReader) throws CorruptIndexException, NoHitsException, IOException {
		Document[] outHits;
		IndexSearcher lSearcher = new IndexSearcher(inReader);
		try {
	 ...
	 */
	public synchronized void waitFor(int inSec) throws InterruptedException {
		long lUntil = System.currentTimeMillis() + inSec * 1000;
		long lSlice = inSec * 100;
	...
	/**
	 * Evaluates the passed <code>TopDocs</code> and fills a <code>Document</code> array with the help of the passed <code>Searcher</code>.
	 * 
	 ...
	 * @throws NoHitsException
	 */
	public static Document[] processSearchResults(TopDocs inHits, IndexSearcher inSearcher) throws CorruptIndexException, IOException, NoHitsException {
		if (inHits.totalHits == 0) throw new NoHitsException();
