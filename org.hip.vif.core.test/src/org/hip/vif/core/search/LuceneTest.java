...
/** @author Benno Luthiger Created on 06.10.2005 */
public class LuceneTest {
    ...
    private int createIndex() throws IOException {
        final IndexWriter lWriter = VIFIndexing.INSTANCE.getMemberIndexWriter(false);
    ...
    private Document createDoc(final int inID, final String inText) {
        final Document outDocument = new Document();
        outDocument.add(new StringField(IDENTITY, String.valueOf(inID), Field.Store.YES));
        outDocument.add(new TextField(SEARCH_TEXT, inText, Field.Store.YES));
    ...
    private Query createQuery(final String inItem) throws ParseException {
        final QueryParser lParser = new QueryParser(SEARCH_TEXT, analyzer);
