...
public class UserTasksView extends CustomComponent {
    ...
    public UserTasksView() {
        ...
        layout.addComponent(new Label(String.format(VIFViewHelper.TMPL_TITLE,
                "vif-pagetitle", messages.getMessage("usersettings.menu.open.tasks")), ContentMode.HTML)); //$NON-NLS-1$ //$NON-NLS-2$
    }

    /** Constructor for view displaying the message that there's no open user task.
     *
     * @param inName String */
    public UserTasksView(final String inName) {
        this();
        layout.addComponent(new Label(
                messages.getFormattedMessage("ui.usersettings.usertasks.no", inName), ContentMode.HTML)); //$NON-NLS-1$
    }
