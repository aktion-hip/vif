...
public class LDAPQueryResultTest {
	...
	public void testDo() throws NamingException, VException, SQLException {
		DirContextWrapper lContext = new TestDirContext();		
		LDAPQueryResult lResult = new LDAPQueryResult(new TestLDAPObjectHome(), lContext.search("(cn=test)", null), EXPECTED.length, null);
	...
	public void testDoAsXML() throws NamingException, VException, SQLException {		
		...
		StringBuffer lTest = new StringBuffer(); 
		while (lResult.hasMoreElements()) {
	...
	@Test
	public void testSerialization() throws NamingException, SQLException, IOException, ClassNotFoundException, VException {
		DirContextWrapper lContext = new TestDirContext();
		
		String lFilter = "(cn=test)";
		LDAPObjectHome lHome = new TestLDAPObjectHome();
		LDAPQueryStatement lStatement = new TestLDAPQueryStatement(lHome);
		lStatement.setSQLString(lFilter);
		
		//1. test: setting cursor two steps ahead
		LDAPQueryResult lResult = new LDAPQueryResult(lHome, lContext.search(lFilter, null), EXPECTED.length, lStatement);
		lResult.next();
		lResult.next();
	...
	private LDAPQueryResult serDes(LDAPQueryResult inResult) throws IOException, ClassNotFoundException {
		...
		LDAPQueryResult outRetrieved = (LDAPQueryResult)lObjectIn.readObject();
		lObjectIn.close();
		lBytesIn.close();
