...
public enum TaskManager {	
	...
	private Map<String, BundleClassLoader> forumTaskMappingTable = new Hashtable<String, BundleClassLoader>();
	private Map<String, BundleClassLoader> adminTaskMappingTable = new Hashtable<String, BundleClassLoader>();
...
// --- bind ---
	 ...
	 */
	public void addForumTaskSet(ITaskSet inTaskSet) {
	 ...
	 * @param inTaskSet {@link ITaskSet}
	 */
	void addAdminTaskSet(ITaskSet inTaskSet) {
		addTaskSet(inTaskSet, adminTaskMappingTable);
	}
	
	private void addTaskSet(ITaskSet inTaskSet, Map<String, BundleClassLoader> inMap) {
		for (ITaskConfiguration lTaskConfiguration : inTaskSet.getTaskConfigurations()) {
			Bundle lBundle = lTaskConfiguration.getBundle();
			String lTaskName = lTaskConfiguration.getTaskName();
			inMap.put(UseCaseHelper.createFullyQualifiedTaskName(lBundle, lTaskName), new BundleClassLoader(lTaskName, lBundle));
		}
	}
...
// --- unbind ---
	 ...
	 * @param inTaskSet
	 */
	public void removeForumTaskSet(ITaskSet inTaskSet) {
	 ...
	 * @param inTaskSet
	 */
	public void removeAdminTaskSet(ITaskSet inTaskSet) {
		removeTaskSet(inTaskSet, adminTaskMappingTable);
	}
	
	private void removeTaskSet(ITaskSet inTaskSet, Map<String, BundleClassLoader> inMap) {
		for (ITaskConfiguration lTaskConfiguration : inTaskSet.getTaskConfigurations()) {
			Bundle lBundle = lTaskConfiguration.getBundle();
			String lTaskName = lTaskConfiguration.getTaskName();
			BundleClassLoader lLoader = inMap.get(UseCaseHelper.createFullyQualifiedTaskName(lBundle, lTaskName));
			if (lLoader != null) {
				lLoader.dispose();
...
// --- task processing ---
	 ...
	 */
	public Component getForumContent(String inTaskName) throws NoTaskFoundException {
	 ...
	 * @throws NoTaskFoundException 
	 */
	public Component getAdminContent(String inTaskName) throws NoTaskFoundException {
	 ...
	 * provided by different bundles are executed.
	 * 
	 ...
	 */
	private Component runTask(BundleClassLoader lTaskLoader) {
		try {
			IPluggableTask lTask = lTaskLoader.createTask();
...
// ---
	...
	private static class BundleClassLoader	{
		...
		public BundleClassLoader(String inTaskName, Bundle inBundle) {
			taskName = inTaskName;
			bundle = inBundle;
		}
		/**
		 ...
		 */
		public IPluggableTask createTask() throws ClassNotFoundException, InstantiationException, IllegalAccessException {
			Class <?> lClass = bundle.loadClass(taskName);
