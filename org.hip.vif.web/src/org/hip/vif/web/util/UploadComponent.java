...
/**
 * Displays file upload button and handles file upload. When the user selects a
 * file to upload and a download file exists already, a popup window is
 * displayed to ask whether the old file has to be replaced by the new one. If
 * no, the newly upload file is discarded.
 * 
...
public class UploadComponent extends CustomComponent {
	 ...
	 * 
	 * @param inDownloads
	 *            {@link QueryResult} the existing download files to display
	 * @param inTask
	...
	private DialogWindow createDialog(final IBibliographyTask inTask) {
		final IMessages lMessages = Activator.getMessages();

		final DialogWindow outDialog = Dialog
				.openQuestion(
	...
	private Component createDropBox(final IBibliographyTask inTask) {
		...
		final Label lHint = new Label(Activator.getMessages().getMessage(
				"ui.upload.drop.box")); //$NON-NLS-1$
	...
	private Upload createUpload(final IBibliographyTask inTask) {
		...
		final String lCaption = Activator.getMessages().getMessage(
				"ui.upload.button.lbl"); //$NON-NLS-1$
	...
	private void handleUpload(final IBibliographyTask inTask,
		...
		if (tempUpload.length() > getMaxSize() * MEGA) {
			handleDeleteTemp();
			Notification
					.show(lMessages.getFormattedMessage(
							"errmsg.upload.exceeded", getMaxSize()), Notification.Type.WARNING_MESSAGE);//$NON-NLS-1$
			return;
		}
		try {
	...
	private long getMaxSize() {
		try {
			...
			LOG.error(
					"Error encountered while retrieving the upload quoata preference!", exc); //$NON-NLS-1$
		}
		catch (final IOException exc) {
			LOG.error(
					"Error encountered while retrieving the upload quoata preference!", exc); //$NON-NLS-1$
