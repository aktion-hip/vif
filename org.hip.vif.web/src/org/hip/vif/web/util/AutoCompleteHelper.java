...
/** Helper class for auto complete fields to look up bibliography items. This class provides either in-memory or lazy
 * loading containers. <br/>
 * If the number of bibliography items exceeds the <code>THRESHOLD</code>, the helper class returns
 * <code>AutoCompleteHelper.LazyLoadingBibliographyContainer</code>.
 *
 * @author Luthiger */
public class AutoCompleteHelper {
     ...
     * @throws VException
     * @throws SQLException */
    public AutoCompleteHelper(final TextHome inHome, final KeyObject inKey) throws VException, SQLException {
    ...
    private void initInMemory(final TextHome inHome, final KeyObject inKey) throws VException, SQLException {
        final Set<String> lTitles = new HashSet<String>();
        final Set<String> lAuthors = new HashSet<String>();
        final QueryResult lTexts = inHome.select(inKey);
        while (lTexts.hasMoreElements()) {
            final GeneralDomainObject lText = lTexts.next();
    ...
    private void initLazyLoading(final TextHome inHome, final KeyObject inKey) {
     ...
     *
     * @author Luthiger Created: 04.09.2011 */
    @SuppressWarnings("serial")
    public static class InMemoryBibliographyContainer extends IndexedContainer {
        InMemoryBibliographyContainer(final List<String> inCaptions) {
            Collections.sort(inCaptions);
            for (final String lCaption : inCaptions) {
                addItem(lCaption);
            }
        }
    }

    /** The container for big data sets loading the data lazy, i.e. after the user entered some characters in the field.
     *
     * @author Luthiger Created: 04.09.2011 */
    @SuppressWarnings("serial")
    public static class LazyLoadingBibliographyContainer extends IndexedContainer {
        ...
        LazyLoadingBibliographyContainer(final TextHome inHome, final String inKeyField) {
            textHome = inHome;
            keyField = inKeyField;
        }

        private void addItems(final List<String> inItems) {
            for (final String lCaption : inItems) {
                addItem(lCaption);
            }
        }
        ...
        @Override
        public Object getIdByIndex(final int inIndex) {
            return filteredItemIds == null ? null : filteredItemIds.get(inIndex);
        }

        @Override
        protected void addFilter(final Filter inFilter) throws UnsupportedFilterException {
            final String lFilterString = getFilterString(inFilter);
        ...
        private String getFilterString(final Filter inFilter) {
