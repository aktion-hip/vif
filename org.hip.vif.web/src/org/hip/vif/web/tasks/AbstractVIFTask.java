...
/**
 * Base class for all tasks of the application. A task class contains the
 * controller and business logic of a use case.
 * 
...
public abstract class AbstractVIFTask implements IPluggableTask {
	...
	/**
	 * Creates a key object that filters all groups in the specified group
	 * states. E.g.
	 * 
	 * <pre>
	...
	protected KeyObject createKey(final Integer[] inStates) throws VException {
		...
		for (int i = 0; i < inStates.length; i++) {
			outKey.setValue(GroupHome.KEY_STATE, inStates[i],
					"=", BinaryBooleanOperator.OR); //$NON-NLS-1$
		}
		return outKey;
	}
	 ...
	 * 
	 * @param inExc
	 *            {@link Exception} the exception causing the problem.
		...
		return (VException) VIFExceptionHandler.getInstance()
				.convert(
						...
						Activator.getMessages().getMessage(
								"errmsg.error.contactAdmin")); //$NON-NLS-1$
	}
	...
	protected VException createNoPermissionException() {
		return new VException(Activator.getMessages().getMessage(
	 ...
	 * 
	 * @param inTaskName
	 *            String the fully qualified name of the next task
	 */
	 ...
	 *            Class the task
	 * @param inIsForum
	 *            boolean <code>true</code> if the task to forward to is a forum
	 *            task, else <code>false</code>
	...
	/**
	 * Sets the <code>EventAdmin</code> to the task for that the task can send
	 * events.
	 * 
	 * @param inEventAdmin
	 *            {@link EventAdmin}
