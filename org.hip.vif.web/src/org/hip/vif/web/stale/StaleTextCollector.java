...
public class StaleTextCollector extends NotificationTextCollector {
    ...
    private final TextParser markupParser = new TextParser();
     ...
     * @throws VException
     * @throws SQLException */
    public void visitQuestion(final QuestionCollector inQuestionCollector) {
        notificationText
                .append(questionMessage)
                .append(" ").append(inQuestionCollector.getDecimal()).append(": ").append(inQuestionCollector.getContributionTitle()).append("\n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        notificationText.append(remarkMessage).append(": ").append(inQuestionCollector.getRemark()).append("\n\n"); //$NON-NLS-1$ //$NON-NLS-2$
        notificationTextHtml
                .append("<p><b>").append(questionMessage).append(" <i>").append(inQuestionCollector.getDecimal()).append("</i>:</b></p>").append(markupParser.parseToHtml(inQuestionCollector.getContributionTitle())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        notificationTextHtml
                .append("<p><b>").append(remarkMessage).append(":</b></p>").append(markupParser.parseToHtml(inQuestionCollector.getRemark())); //$NON-NLS-1$ //$NON-NLS-2$
    }
     ...
     * @throws VException
     * @throws SQLException */
    public void visitCompletion(final CompletionCollector inCompletion) {
        notificationText
                .append(completionMessage)
                .append(" (").append(inCompletion.getDecimalID()).append("): ").append(inCompletion.getContributionTitle()).append("\n\n"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
        notificationTextHtml
                .append("<p><b>").append(completionMessage).append(" <i>(").append(inCompletion.getDecimalID()).append(")</i>:</b></p>").append(markupParser.parseToHtml(inCompletion.getContributionTitle())); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
    }

    public void visitText(final TextCollector inText) {
        notificationText.append(bibliographyMessage).append("\n\n").append(inText.getContentPlain()); //$NON-NLS-1$
        notificationTextHtml
                .append("<p><b>").append(bibliographyMessage).append(":</b></p>").append(inText.getContentHtml()); //$NON-NLS-1$ //$NON-NLS-2$
    }
