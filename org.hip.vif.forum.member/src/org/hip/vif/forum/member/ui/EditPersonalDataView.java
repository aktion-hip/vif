...
public class EditPersonalDataView extends CustomComponent {
    ...
    public EditPersonalDataView(final Member inMember, RatingsHelper inRatings, final PersonalDataEditTask inTask) {
        ...
        lLayout.addComponent(new Label(
                String.format(VIFViewHelper.TMPL_TITLE,
                        ...
                        lMessages
                                .getFormattedMessage(
                                        "ui.member.view.title.page", BeanWrapperHelper.getString(MemberHome.KEY_USER_ID, inMember))), Label.CONTENT_XHTML)); //$NON-NLS-1$ //$NON-NLS-2$
        ...
        TextField lField = VIFViewHelper.setRequired(createTextField(inMember, MemberHome.KEY_FIRSTNAME),
                new MemberRepaintRequestListener());
        lField.focus();
        lTable.addRowEmphasized(lMessages.getMessage("ui.member.editor.label.firstname"), lField); //$NON-NLS-1$
        lTable.addRowEmphasized(
                lMessages.getMessage("ui.member.editor.label.name"), VIFViewHelper.setRequired(createTextField(inMember, MemberHome.KEY_NAME), new MemberRepaintRequestListener())); //$NON-NLS-1$
        lTable.addRowEmphasized(
                lMessages.getMessage("ui.member.editor.label.street"), VIFViewHelper.setRequired(createTextField(inMember, MemberHome.KEY_STREET), new MemberRepaintRequestListener())); //$NON-NLS-1$
        lTable.addRowEmphasized(
                lMessages.getMessage("ui.member.editor.label.city"), createZipCityFields(inMember, lMessages)); //$NON-NLS-1$
        lTable.addRow(
                lMessages.getMessage("ui.member.editor.label.phone"), createTextField(inMember, MemberHome.KEY_PHONE)); //$NON-NLS-1$
        lTable.addRow(lMessages.getMessage("ui.member.editor.label.fax"), createTextField(inMember, MemberHome.KEY_FAX)); //$NON-NLS-1$
        lTable.addRowEmphasized(
                lMessages.getMessage("ui.member.editor.label.mail"), VIFViewHelper.setRequired(createTextField(inMember, MemberHome.KEY_MAIL), new MemberRepaintRequestListener())); //$NON-NLS-1$
                ...
                if (!inMember.isValid()) {
                    ...
                    if (inTask.saveMember(inMember)) {
                        getWindow().showNotification(
                                lMessages.getMessage("msg.task.data.changed"), Notification.TYPE_TRAY_NOTIFICATION); //$NON-NLS-1$
                    }
                    else {
                        getWindow().showNotification(
                                lMessages.getMessage("errmsg.save.general"), Notification.TYPE_WARNING_MESSAGE); //$NON-NLS-1$						
                    }
                }
            }
        });
    ...
    private Component createZipCityFields(Member inMember, IMessages inMessages) {
        ...
        outLayout.addComponent(VIFViewHelper.setRequired(
                VIFViewHelper.createTextField(inMember, MemberHome.KEY_CITY, 239), new MemberRepaintRequestListener()));
        return outLayout;
    }
    ...
    private class MemberRepaintRequestListener implements RepaintRequestListener {
        public void repaintRequested(RepaintRequestEvent inEvent) {
            ...
            if (lField.isValid()) {
                ...
                getWindow().showNotification(
                        Activator.getMessages().getMessage("errmsg.field.not.empty"), Notification.TYPE_ERROR_MESSAGE); //$NON-NLS-1$
                lField.focus();
            }
        }
    }
    ...
    private static class MemberFieldChecker extends MandatoryFieldChecker {
        public MemberFieldChecker(Member inMember, IMessages inMessages) {
            ...
            checkMandatory(inMember, MemberHome.KEY_FIRSTNAME,
                    inMessages.getMessage("ui.member.editor.label.firstname")); //$NON-NLS-1$
