...
public class EditPwrdView extends CustomComponent {
    ...
    public EditPwrdView() {
        ...
        lLayout.addComponent(new Label(String.format(VIFViewHelper.TMPL_TITLE,
                "vif-pagetitle", "ui.pwrd.view.title.disabled"), Label.CONTENT_XHTML)); //$NON-NLS-1$ //$NON-NLS-2$
    }
            ...
            public void buttonClick(ClickEvent inEvent) {
                if (new PasswordInputChecker(lNew1, lNew2, getWindow()).checkInput()) {
                    try {
                        ...
                        getWindow().showNotification(
                                lMessages.getMessage("errmsg.pwrd.invalid"), Notification.TYPE_WARNING_MESSAGE); //$NON-NLS-1$
                    }
                }
            }
        });
