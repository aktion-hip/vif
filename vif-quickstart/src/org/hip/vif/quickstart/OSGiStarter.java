...
public class OSGiStarter {
	 ...
	 */
	private OSGiStarter(String inPort, File inWorkingDir) {
		workingDir = inWorkingDir;
		FrameworkFactory lFactory = ServiceLoader.load(FrameworkFactory.class).iterator().next();
		
		Map<String, String> lConfiguration = new HashMap<String, String>();
//		lConfiguration.put(org.osgi.framework.Constants.FRAMEWORK_STORAGE_CLEAN, "true");
	...
	public void installBundles() {
		File lLookupDir = new File(workingDir, Constants.PLUGINS_DIR);
		File[] lBundleFiles = lLookupDir.listFiles(new FilenameFilter() {			
			@Override
		...
		BundleContext lContext = framework.getBundleContext();
		List<Bundle> lInstalledBundles = new LinkedList<Bundle>();
		for (File lFile : lBundleFiles) {
			try {
		...
		for (Bundle lBundle : lInstalledBundles) {
			try {
	 ...
	 */
	public static OSGiStarter getOSGi(String inPort, File inWorkingDir) {
